...
(defmethod exec :readln [{:keys [mem pc in-chan] :as prog} [r]]
  (let [v (or (and in-chan (BigInteger. (str (<!! in-chan))))
              (do
...
(defmethod exec :putln [{:keys [pc out-chan] :as prog} [a]]
  (let [v (:val a)
        pc (+ pc 2)]
    (if out-chan
      (assert (>!! out-chan v))
...
(defmethod exec :jump-if-true [{:keys [pc] :as prog} [a b]]
  (let [[av bv] (map :val [a b])
        pc (if ((complement zero?) av) bv (+ pc 3))]
    (assoc prog :pc pc)))

(defmethod exec :jump-if-false [{:keys [pc] :as prog} [a b]]
  (let [[av bv] (map :val [a b])
